...
More cleaning of the event type field
```{r morecleaneventtype}
dog_df$type_of_dog <- revalue (dog_df$type_of_dog, c("WELSH CORGI"="CORGI", "PEMBROKE CORGI"="CORGI","THAT DOG THE QUEEN HAS"="CORGI")
...
winterdata <- stormdata %>%
    ...
    filter(winter==T | snow==T | cold==T | blizz==T) %>%
    mutate(evtype = "winter") %>%
    select(-winter,-snow,-cold,-blizz)



table(stormdata$evtype, useNA = "always")

           search = c(grep("wint"), grep("snow"), grep("cold"), grep("blizzard")),
            replace = c("winter", "winter", "winter", "winter"))
            
            search = c("tstm", "tropical", "tunder", "tsunami"),
            replace = c("tstorm", "tstorm", "tstorm", "tstorm") %>%
           ...
           tstorm = str_detect(evtype, ),
           wind = str_detect(evtype, "wind"),
           hail = str_detect(evtype, "hail"),
           ice = str_detect(evtype, "ice"),
           fire = str_detect(evtype, "fire"),
           dry = str_detect(evtype, "drought" | "dry"),
           fog = str_detect(evtype, "fog"),
           tornado = str_detect(evtype, "tornado" | "torndao")) 
