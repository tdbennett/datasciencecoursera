...
7) Clean the state field.  Because the question relates to the U.S., I excluded rows describing events that did not have
a two-letter code for one of the 50 U.S. states
```{r cleanstate}
...
semi_join(stormdata,state2)

badstates <- c("GM", "GU", "XX", "LC", "LE", "LH", "LM", "LO", "LS", "MH", "PH", "PK", "PM", "PR", "PZ", "SL", "ST", "VI")
stormdata <- stormdata %>%
    filter(state.abb - %in% badstates)

stormdata$state.abb[stormdata$state.abb=="XX"] <- NA
stormdata$state.abb[stormdata$state.abb=="SL"] <- NA
stormdata$state.abb[stormdata$state.abb=="ST"] <- NA
```
  


7) Clean the event type field
