...
with(mtcars, boxplot(mpg ~ trtype, main = "Miles per gallon, by transmission type"))
```
...
Any of the other 9 variables in the `mtcars` dataset might be hypothesized to be important, but 
to avoid modeling nuisance variables I will only consider those that have a relationship with the outcome, mileage.
```{r econimpact}
bytrtype <- mtcars %>%
    group_by(trtype) %>%
    summarize(
