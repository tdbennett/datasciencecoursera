...
of the 9 variables against `mpg` shown in Appendix [Figure 2][fig2] suggest that any 
...
The residual plot (Appendix [Figure 4][fig4]) suggests that our final model fits the data reasonably well. The
...
The updated residual plots (Appendix [Figure 5][fig 5] suggest a better fit with point 17 removed. 
...
```{r bivplot, cache=TRUE, fig.height=3, fig.width=6}
boxplot(mpg ~ trtype, main = "MPG, by transmission type", data = mtcars)
```

[fig1]: Project_files/figure-latex/bivplot-1.pdf "Figure 1"
...
[fig2]: Project_files/figure-latex/bivariateplots-1.pdf "Figure 2"

## Figure 3 code

```{r pairsplot, cache = TRUE, echo = TRUE}
data(mtcars)
p <- ggpairs(mtcars, columns = 2:11, lower = list(continuous = "smooth"), params = c(method = "loess"))
```

[fig3]: Project_files/figure-latex/pairsplot-1.pdf "Figure 3"

## Figure 4 - Residual plots from Model 4

```{r resids, fig.height = 4}
par(mfrow = c(2,2))
plot(fit4)
par(mfrow = c(1,1))
```

[fig4]: Project_files/figure-latex/pairsplot-1.pdf "Figure 4"

## Figure 5 - Residual plots from Final Model

```{r resids2, fig.height = 4}
par(mfrow = c(2,2))
plot(fit5)
par(mfrow = c(1,1))
```

[fig5]: Project_files/figure-latex/pairsplot-1.pdf "Figure 5"
