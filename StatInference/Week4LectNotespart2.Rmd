...
---
## Notes from B Caffo week 4 bootstrapping lectures
...
```{r}
B <- 10000  ### want B to be large, to make Monte Carlo error (not enough sims) small
resamples <- matrix(sample(x,
                           n * B,  ### sample of n obs from a data set of n obs
                           replace = TRUE),  ### with replacement to avoid just getting the original data set 
                    B, n)
...
- The bootstrap shown here is non-parametric
- Better percentile bootstrap confidence intervals correct for bias 
    - look for the bias-corrected accelerated (BCA) interval in the bootstrap package in R
...
observedStat
mean(permutations > observedStat) ### percentage of permuted test stats that are larger than observed statistic
```
