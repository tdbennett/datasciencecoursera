...
### Asymptotics and the law of large numbers
...
```{r lln2, cache=TRUE}
means <- cumsum(sample(0 : 1, n , replace = TRUE)) / (1 : n) ### flipping a coin 1000 times
g <- ggplot(data.frame(x = 1 : n, y = means), aes(x = x, y = y)) 
...
* An estimator is __consistent__ if it converges to what you want to estimate
...
* TB: it doesn't matter what the underlying distribution is
...
TB: speed with which the distr'n converges to normal is related to p (slower if p farther from 0.5)

```{r cltcoin2, cache=TRUE}
nosim <- 1000
...
* Replacing $p$ by $\hat p$ in the standard error results in what is called a __Wald__ confidence interval for $p$
* For 95% intervals $$\hat p \pm \frac{1}{\sqrt{n}}$$ is a quick CI estimate for $p$ - TB very useful
...
`r .56 + c(-1, 1) * qnorm(.975) * sqrt(.56 * .44 / 100)`  

`r binom.test(56, 100)$conf.int` ### TB - this is the exact binomial CI, doesn't rely on CLT

### Simulation - Wald CI

```{r simbinom, cache=TRUE}
n <- 20
pvals <- seq(.1, .9, by = .05)
nosim <- 1000
...
ggplot(data.frame(pvals, coverage), aes(x = pvals, y = coverage)) + geom_line(size = 2) + geom_hline(yintercept = 0.95) + ylim(.75, 1.0) ### this picture shows that the CLT (and Wald) is most accurate at p=0.5, much less at edges
```
...
* (Add two successes and failures, __Agresti/Coull__ interval)

### Simulation

* First let's show that coverage (of the true prob) gets better with $n$

```{r sim20}
n <- 100 ### rather than 20
pvals <- seq(.1, .9, by = .05)
nosim <- 1000
...
### Simulation - Agresti/Coull, he says use this one all the time

* Now let's look at $n=20$ but adding 2 successes and failures (Agresti/Coull interval from 2 slides up)
...
poisson.test(x, T = 94.32)$conf ### this is an exact Poisson interval - guarantees 95% coverage
```
